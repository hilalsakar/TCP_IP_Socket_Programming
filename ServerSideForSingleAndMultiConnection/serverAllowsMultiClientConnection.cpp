...
DWORD WINAPI ProcessClient(LPVOID lpParam) {
	...
	printf("New thread was created, it's ID: %d\n", threadId);
	int iResult;
	...
	iResult = recv(ClientSocket, recvbuf, sizeof(recvbuf), MSG_WAITALL);
	printf("Received: %c\n", *recvbuf);

fin:
	printf("Client(Thread %d) processed.\n", threadId);
	return 0;
...
int main() {
	...
	while (1) {
		...
		DWORD threadId;
		CreateThread(NULL, 0, ProcessClient, (LPVOID)ClientSocket, 0, &threadId);
	}
